...
public class PantallaJuego implements Pantalla {
	...
	private void reproducirDisparo() {
		try {
			AudioInputStream audioInputStream = AudioSystem
					.getAudioInputStream(new File("sonidos/atat.wav").getAbsoluteFile());
	...
	private void reproducirMusica() {
		try {
			AudioInputStream audioInputStream = AudioSystem
					.getAudioInputStream(new File("sonidos/musica.wav").getAbsoluteFile());
			...
			if (asteroides.get(i).colisionan(disparo)) {
				...
				if (asteroides.get(i).getAncho() == ANCHO_ASTEROIDE) {
					for (int j = 0; j < 2; j++) {
						int velocidadX = rd.nextInt(10) + 1;
						int velocidadY = rd.nextInt(10) + 1;
						Sprite mitad = new Sprite(asteroides.get(i).getAlto() / 2, asteroides.get(i).getAncho() / 2,
								asteroides.get(i).getPosX(), asteroides.get(i).getPosY(), velocidadX, velocidadY,
								"Imagenes/asteroide.png");
	...
	private void elegirPowe() {
		...
		case 1:
			for (int j = 0; j < asteroides.size(); j++) {
				...
				asteroides.get(j).setVelocidadY(asteroides.get(j).getVelocidadY() / 2);
			}
			break;
		case 2:
